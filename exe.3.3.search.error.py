...
from class_databace_myself import Database
root = Tk()
root.title('')
root.geometry('575x450+150+150')
root.resizable(0,0)
...
def delete(): 
    ...
    try:
        index=index
        if index == -1.5:
            messagebox.showerror('EMPTY','you have not chose any thing to delete')
            return
    except:
        messagebox.showerror('EMPTY','you have not chose any thing to delete')
        return
...
def clear():
    ...
    e5.delete(0,END)
def exit1 ():
    result=messagebox.askyesno('EXIT','are you sure about closing this window?')
    if result== False:
        return
    messagebox.showinfo("BYE",'have a nice time ;)')
    root.destroy()
def close(event):
    t1=Tk()
    t1.title('')
